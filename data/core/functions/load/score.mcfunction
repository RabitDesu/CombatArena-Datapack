#> core:load/score
#  Set scoreboard object

scoreboard objectives add Const dummy
scoreboard objectives add Temp dummy

scoreboard objectives add PlayerID dummy
scoreboard objectives add EntityID dummy

scoreboard objectives add DeathTimer dummy

scoreboard objectives add join custom:play_time
scoreboard objectives add quit custom:leave_game
scoreboard objectives add sneaktime custom:sneak_time

scoreboard objectives add Class dummy

scoreboard objectives add HP dummy
scoreboard objectives add HPMax dummy
scoreboard objectives add AGI dummy

scoreboard objectives add BaseHPMax dummy
scoreboard objectives add BaseAGI dummy

scoreboard objectives add BonusHPMax dummy
scoreboard objectives add BonusAGI dummy